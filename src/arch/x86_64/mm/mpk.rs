...
use arch::x86_64::mm::paging::BasePageSize;
use arch::x86_64::kernel::processor;

const MPK_PAGE_SIZE: usize = 4096;
const EINVAL: i32 = 22;
const ENOSYS: i32 = 38;
...
pub fn mpk_mem_set_key(mut addr: usize, mut size: usize, key: u8) -> i32 {
    ...
    addr = (addr) & !(MPK_PAGE_SIZE-1);
    /* If needed ceil [addr + size[ to the nearest page */
    if ((MPK_PAGE_SIZE-1)&size) > 0 {
        size = (size + MPK_PAGE_SIZE) & !(MPK_PAGE_SIZE-1);
    }

    let mut count :usize = size/MPK_PAGE_SIZE;
    if size%MPK_PAGE_SIZE > 0
    {
        count = count + 1;
    }
